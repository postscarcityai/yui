...
try:
    ...
    check("host marks the person's row delivered and handled", s == 200 and len(r) == 1 and bool(r[0]["handled_at"]), f"{s} {r}")
    s, r = rest("PATCH", f"yui_messages?id=eq.{a_msg}", ctA, {"handled_at": now, "body": "edited"})
    check("host still cannot touch the body while acking", s in (401, 403), f"{s} {code(r)}")
    s, r = rest("PATCH", f"yui_messages?id=eq.{a_msg}", tokA, {"handled_at": None})
    check("app cannot set the host's acks", s in (401, 403), f"{s} {code(r)}")
    s, r = rest("PATCH", f"yui_messages?id=eq.{a_msg}", ctB, {"handled_at": now}, prefer="return=representation")
    check("another user's host cannot ack A's rows", s == 200 and r == [] or s in (401, 403), f"{s} {r}")
    mine = str(uuid.uuid4())
    s, r = rest("POST", "yui_messages", ctA, {**msg(A, a_agent, sender="agent", body="queued reply"), "id": mine})
    check("host chooses its reply's id", s == 201, f"{s} {code(r)}")
    s, r = rest("POST", "yui_messages", ctA, {**msg(A, a_agent, sender="agent", body="queued reply"), "id": mine})
    check("a resend with the same id is refused, not written twice (409)", s == 409, f"{s} {code(r)}")
    s, r = rest("PATCH", f"yui_messages?id=eq.{mine}", ctA, {"handled_at": now}, prefer="return=representation")
    check("host cannot ack its own replies (only the person's rows)", s == 200 and r == [] or s in (401, 403), f"{s} {r}")
    s, r = rest("GET", "yui_messages?select=id,sender,kind", ctA)
    ...
    print("== Goodbye: a clean stop reads offline, a heartbeat brings it back (YUI-28)")
    def presence(): return rest("GET", f"yui_agent_list?select=presence,status&id=eq.{a_agent}", tokA)[1][0]
    s, r = host({"action": "bye"}, a_ct)
    check("host says goodbye: offline at once", s == 200 and presence() == {"presence": "offline", "status": "offline"},
          f"{s} {presence()}")
    host({"action": "heartbeat"}, a_ct)
    check("its next heartbeat reads online", presence() == {"presence": "online", "status": "connected"}, f"{presence()}")
    sql(f"update yui_connectors set last_seen_at = now() - interval '3 minutes' where id = '{a_cid}'")
    check("quiet without a goodbye reads asleep", presence() == {"presence": "asleep", "status": "offline"}, f"{presence()}")
    s, r = host({"action": "bye"}, "yui_ct_not-a-real-token")
    check("goodbye needs the connector token", s == 401, f"{s}")
